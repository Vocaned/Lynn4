...
class Scripting(lynn.Plugin):
    def __init__(self, bot: lynn.Bot):
        super().__init__(bot)

        # Overwrite the default message handler
        bot.unsubscribe(hikari.MessageCreateEvent, bot.handle)
        bot.subscribe(hikari.MessageCreateEvent, self.handle)

    def plugin_remove(self):
        # Revert to the default message handler when the plugin is removed
        self.bot.unsubscribe(hikari.MessageCreateEvent, self.handle)
        self.bot.subscribe(hikari.MessageCreateEvent, self.bot.handle)

    async def handle(self, event: hikari.MessageCreateEvent) -> None:
        if self.bot.config.get('typingindicator'):
            async with self.bot.rest.trigger_typing(event.channel_id):
                await self.handler(event)
        else:
            await self.handler(event)

    async def handler(self, event: hikari.MessageCreateEvent) -> None:
        if self.bot.ignore_bots and not event.is_human:
            return
...
def load(bot: lynn.Bot):
    bot.add_plugin(Scripting(bot))

def unload(bot: lynn.Bot):
    bot.remove_plugin('Scripting')
